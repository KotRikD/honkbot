...
async def convert_to_message(update, eenv):
    ...
    if "conversation_message_id" in obj:
        ...
        try:
            pl = json.loads(update['object'].get("payload", "{}"))
        except:
            pl = dict()

        if type(pl) == dict and pl.get("command", None):
            text = pl['command']
            eenv["is_payload"] = True
        else:
            eenv["is_payload"] = False

    text = text.replace("@all", "").replace("@online", "")
